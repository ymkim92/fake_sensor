...
def test_version_initialization() -> None:
...
def test_version_to_bytes() -> None:
    version = Version(1, 2, 3, 0x1ABCDEF0)
    assert version.to_bytes() == b"\x01\x02\x03\x1a\xbc\xde\xf0"


def test_version_from_bytes() -> None:
...
def test_invalid_major_minor_patch() -> None:
...
def test_invalid_git_sha() -> None:
    with pytest.raises(ValueError):
        Version(1, 2, 3, 0x100000000)


def test_invalid_from_bytes() -> None:
    with pytest.raises(ValueError):
